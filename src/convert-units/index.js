  ...
  */
  each(this.possibilities(), function(possibility) {
    ...
    if (isIncluded && unit.system === this.origin.system) {
      ...
      if ((!best && result >= options.cutOffNumber ) || (result >= options.cutOffNumber && result < best.val)) {
        best = {
          val: result,
          unit: possibility,
  ...
  if (!best){
    var possibility = this.possibilities()[0];
    best = {
      val: this.to(possibility),
      unit: possibility,
      singular: this.describe(possibility).singular,
      plural: this.describe(possibility).plural
    };
  }
